...
# 重新设置INPUT_DIR为正确的路径
os.environ['INPUT_DIR'] = os.path.join(project_root, 'input')
...
if __name__ == "__main__":
    ...
    if not os.path.exists(switch_file):
        missing_files.append(INPUT_SWITCH_FILE)
    if not os.path.exists(power_file):
        missing_files.append(INPUT_POWER_FILE)
        
    if missing_files:
        print("警告：输入文件缺失！")
        print("请确保以下文件存在于", INPUT_DIR, "目录中：")
