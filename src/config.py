...
# 输入输出目录 - 从环境变量获取或使用默认值
INPUT_DIR = os.environ.get('INPUT_DIR', os.path.join(PROJECT_DIR, "input"))
...
def get_target_meters():
    ...
    if os.path.exists(power_file_path):
        ...
        print("INPUT_DIR内容:", files)
        print("文件是否存在:", os.path.exists(power_file_path))
