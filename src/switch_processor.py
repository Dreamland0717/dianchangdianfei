...
def adjust_time(original_time: str | datetime, prev_status: int, current_status: int) -> datetime:
    ...
    # 判断是否为15分钟整点
    is_15_min_point = (dt.minute % 15 == 0) and (dt.second == 0) and (dt.microsecond == 0)
    
    # 如果是15分钟整点，则不进行处理
    if is_15_min_point:
        logger.info(f"时间点 {dt} 是15分钟整点，不进行调整")
        return dt
    
    # 如果不是15分钟整点，则进行调整：
    # 状态由1变0时，将该时间点调整为该15分钟区间的开始时间
    # 状态由0变1时，将该时间点调整为该15分钟区间的结束时间
    if prev_status == 1 and current_status == 0:
        # 从运行到停机：取当前15分钟时段的开始时间
        adjusted_time = base_time
        logger.info(f"将时间从 {dt} 调整为 {adjusted_time} (状态变化: {prev_status}→{current_status})")
        return adjusted_time
    elif prev_status == 0 and current_status == 1:
        # 从停机到运行：取当前15分钟时段的结束时间（开始时间+15分钟）
        adjusted_time = base_time + timedelta(minutes=15)
        logger.info(f"将时间从 {dt} 调整为 {adjusted_time} (状态变化: {prev_status}→{current_status})")
        return adjusted_time
...
def extract_downtime_intervals_and_changepoints(switch_file: str) -> Tuple[List[Tuple[datetime, datetime]], List[Tuple[datetime, int]]]:
    """提取停机时间段和启停状态变化点"""
    try:
        ...
        # 检查是否有数据
        if len(df_switch) == 0:
            logger.warning("启停数据文件为空")
            return [], []
        
        # 检查启停标识是否有效
        # 确保启停标识是数值类型
        df_switch['启停标识'] = pd.to_numeric(df_switch['启停标识'], errors='coerce')
        
        # 检查是否有无效的启停标识
        valid_status = df_switch['启停标识'].isin([0, 1])
        invalid_count = (~valid_status).sum()
        if invalid_count > 0:
            logger.warning(f"发现 {invalid_count} 条无效的启停标识记录")
            # 只保留有效的记录
            df_switch = df_switch[valid_status].copy()
            if len(df_switch) == 0:
                logger.error("没有有效的启停标识记录")
                return [], []
        
        # 按时间排序
        df_switch = df_switch.sort_values('启停时间').reset_index(drop=True)
        
        downtime_intervals = []
        current_start = None
        change_points = []  # 存储所有启停状态变化点及其状态
        
        # 记录第一个状态变化点
        if len(df_switch) > 0:
            change_points.append((df_switch.iloc[0]['启停时间'], int(df_switch.iloc[0]['启停标识'])))
        
        logger.info("开始提取停机时间段和变化点...")
        for i in range(1, len(df_switch)):
            prev_row = df_switch.iloc[i-1]
            current_row = df_switch.iloc[i]
            
            prev_status = int(prev_row['启停标识'])  # 转换为整数
            current_status = int(current_row['启停标识'])  # 转换为整数
            current_time = current_row['启停时间']
            
            # 如果状态发生变化，记录变化点
            if prev_status != current_status:
                change_points.append((current_time, current_status))
                logger.info(f"发现状态变化点: {current_time} ({prev_status}→{current_status})")
            
            # 构建停机时间段 (状态为0表示停机)
            # 当状态从1变为0时，停机开始
            if prev_status == 1 and current_status == 0:
                current_start = current_time
                logger.info(f"停机开始: {current_start}")
            # 当状态从0变为1时，停机结束
            elif prev_status == 0 and current_status == 1 and current_start is not None:
                downtime_intervals.append((current_start, current_time))
                logger.info(f"停机结束: {current_start} 至 {current_time}")
                current_start = None
        ...
        if current_start is not None:
            ...
            logger.info(f"添加未结束的停机时间段: {current_start} 至 {end_date}")
        
        logger.info(f"提取到 {len(downtime_intervals)} 个停机时间段和 {len(change_points)} 个变化点")
        return downtime_intervals, change_points
    
    except Exception as e:
        logger.error(f"处理启停数据文件时出错: {e}")
...
def process_switch_data():
    ...
    try:
        ...
        # 确保有足够的列
        if len(df.columns) < 4:
            logger.error(f"文件列数不足，至少需要4列，当前只有{len(df.columns)}列")
            return None, None
            
        # 保存原始列名
        original_columns = list(df.columns)
        logger.info(f"文件列名: {original_columns}")
        
        # 确保时间列是datetime类型
        time_column = df.columns[2]  # 假设第3列是启停时间
        status_column = df.columns[3]  # 第4列是启停标识
        
        logger.info(f"时间列: {time_column}, 状态列: {status_column}")
        logger.info(f"时间列数据类型: {df[time_column].dtype}")
        logger.info(f"状态列数据类型: {df[status_column].dtype}")
        
        if not pd.api.types.is_datetime64_any_dtype(df[time_column]):
            logger.info("转换时间列为datetime类型...")
        ...
        for i in range(1, len(df)):
            ...
            # 检查状态值是否有效
            if pd.isna(prev_status) or pd.isna(current_status):
                logger.warning(f"发现空状态值: 行 {i-1}={prev_status}, 行 {i}={current_status}")
                continue
            ...
            if prev_status not in [0, 1] or current_status not in [0, 1]:
                logger.warning(f"发现无效状态值: 行 {i-1}={prev_status}, 行 {i}={current_status}")
                continue
        ...
        # 执行删除操作
        if indices_to_drop:
            logger.info(f"删除 {len(indices_to_drop)} 条重复状态记录")
        ...
        logger.error(f"处理启停数据时发生未预期的错误: {e}")
        logger.error(traceback.format_exc())
        print(f"处理启停数据时发生错误: {e}")
        traceback.print_exc()
        return None, None